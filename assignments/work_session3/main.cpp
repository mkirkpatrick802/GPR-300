...
int main() {
	...
	ew::Shader toon = ew::Shader("assets/pbr_toon.vert", "assets/pbr_toon.frag");
	...
	GLuint toon_index = ew::loadTexture("assets/AZtoon.png");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// Render Shell
		{
			...
			glBindTextureUnit(5, toon_index);

			// Set uniforms
			toon.use();
			toon.setInt("materials.color", 0);
			toon.setInt("materials.albedo", 1);
			toon.setInt("materials.metallic", 2);
			toon.setInt("materials.occulusion", 3);
			toon.setInt("materials.specular", 4);
			toon.setVec3("eye_position", camera.position);
			toon.setVec3("ambient_color", ambient_color);

			toon.setMat4("model_matrix", shellTransform.modelMatrix());
			toon.setMat4("projection_matrix", camera.projectionMatrix() * camera.viewMatrix());

			toon.setInt("toon_index", 5);
